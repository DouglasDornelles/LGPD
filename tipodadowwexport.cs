...
namespace GeneXus.Programs {
   ...
   public class tipodadowwexport__default : DataStoreHelperBase, IDataStoreHelper
   {
                                             ...
                                             bool AV18OrderedDsc )
      {
         ...
         {
            AddWhere(sWhereString, "(( [TipoDadoNome] like '%' + @lV46Tipodadowwds_1_filterfulltext) or ( 'sim' like '%' + LOWER(@lV46Tipodadowwds_1_filterfulltext) and [TipoDadoAtivo] = 1) or ( 'não' like '%' + LOWER(@lV46Tipodadowwds_1_filterfulltext) and [TipoDadoAtivo] = 0))");
         }
         else
         {
            GXv_int3[0] = 1;
