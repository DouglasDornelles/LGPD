...
namespace GeneXus.Programs {
   public class arearesponsavelprompt : GXDataArea, System.Web.SessionState.IRequiresSessionState
   {
      ...
      protected void StartGridControl26( )
      {
         if ( GridContainer.GetWrapped() == 1 )
         {
            ...
            context.SendWebValue( "Área Responsável") ;
            context.WriteHtmlTextNl( "</th>") ;
            context.WriteHtmlText( "<th align=\""+"left"+"\" "+" nowrap=\"nowrap\" "+" class=\""+"Attribute"+"\" "+" style=\""+""+""+"\" "+">") ;
   ...
   public class arearesponsavelprompt__default : DataStoreHelperBase, IDataStoreHelper
   {
                                             ...
                                             bool AV12OrderedDsc )
      {
         ...
         {
            AddWhere(sWhereString, "(( CONVERT( char(8), CAST([AreaResponsavelId] AS decimal(8,0))) like '%' + @lV13FilterFullText) or ( [AreaResponsavelNome] like '%' + @lV13FilterFullText) or ( 'sim' like '%' + LOWER(@lV13FilterFullText) and [AreaResponsavelAtivo] = 1) or ( 'não' like '%' + LOWER(@lV13FilterFullText) and [AreaResponsavelAtivo] = 0))");
         }
         else
         {
            GXv_int3[0] = 1;
