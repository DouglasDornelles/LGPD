...
namespace GeneXus.Programs {
   ...
   public class temporetencaowwexport__default : DataStoreHelperBase, IDataStoreHelper
   {
                                             ...
                                             bool AV18OrderedDsc )
      {
         ...
         {
            AddWhere(sWhereString, "(( [TempoRetencaoNome] like '%' + @lV46Temporetencaowwds_1_filterfulltext) or ( 'sim' like '%' + LOWER(@lV46Temporetencaowwds_1_filterfulltext) and [TempoRetencaoAtivo] = 1) or ( 'não' like '%' + LOWER(@lV46Temporetencaowwds_1_filterfulltext) and [TempoRetencaoAtivo] = 0))");
         }
         else
         {
            GXv_int3[0] = 1;
