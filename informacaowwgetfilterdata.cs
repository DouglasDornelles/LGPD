...
namespace GeneXus.Programs {
   ...
   public class informacaowwgetfilterdata__default : DataStoreHelperBase, IDataStoreHelper
   {
                                             ...
                                             bool A71InformacaoAtivo )
      {
         ...
         {
            AddWhere(sWhereString, "(( [InformacaoNome] like '%' + @lV43Informacaowwds_1_filterfulltext) or ( 'sim' like '%' + LOWER(@lV43Informacaowwds_1_filterfulltext) and [InformacaoAtivo] = 1) or ( 'não' like '%' + LOWER(@lV43Informacaowwds_1_filterfulltext) and [InformacaoAtivo] = 0))");
         }
         else
         {
            GXv_int1[0] = 1;
